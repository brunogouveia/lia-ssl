...
class CMatrix {
	...
	public:
		...
		double Determinant() {
			...
			switch (m_rows) {
				...
				case 4: {
					...
					double det = this->m_pData[0][0] * temp[0]->Determinant() - this->m_pData[0][1] * temp[1]->Determinant() + this->m_pData[0][2] * temp[2]->Determinant() - this->m_pData[0][3] * temp[3]->Determinant();
					return det;
				}
					break;
				...
				case 5: {
					...
					double det = this->m_pData[0][0] * temp[0]->Determinant() - this->m_pData[0][1] * temp[1]->Determinant() + this->m_pData[0][2] * temp[2]->Determinant() - this->m_pData[0][3] * temp[3]->Determinant() + this->m_pData[0][4] * temp[4]->Determinant();
					return det;
				}
				case 6:
				case 7:
		...
		CMatrix operator *(const float other) {

			CMatrix result("", this->m_rows, this->m_cols);

			for (int i = 0; i < this->m_rows; i++) {
				for (int j = 0; j < this->m_cols; j++) {
					for (int k = 0; k < this->m_cols; k++) {
						result.m_pData[i][j] += this->m_pData[i][k] * other;
					}
				}
			}
			return result;
		...
		static CMatrix identity(int dimension) {
			CMatrix ident("", dimension, dimension);
