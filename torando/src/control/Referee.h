...
class Referee: public TimerModule {
	...
	private:
		...
		virtual void doInBackground() {
			...
			std::cout << "TS=" << status.packet_timestamp() << ", stage=" << status.stage() << ", stage_time_left=" << status.stage_time_left() << ", command=" << status.command() << ", yscore=" << status.yellow().score() << ", bscore=" << status.blue().score() << '\n';
